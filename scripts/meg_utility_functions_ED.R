...
meg_ordination <- function(data_list,
    ...
    for( l in 1:length(local_obj) ) {
        ...
        all_ord <- rbind(all_ord, ord_points,fill=TRUE) ### ED, adding fill=TRUE
...
meg_heatmap <- function(melted_data,
                        ...
                        outdir,
                        data_type) {
    tile_subset <- melted_data[melted_data$Level_ID == level_var,] # added melted_data within the brackets
    ...
    sample_order <- unique(tile_subset[order(group_var), sample_var,with=FALSE])
